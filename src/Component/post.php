            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                if ($this->posts->num_rows > 0) {
                    while ($row = $this->posts->fetch_assoc()) {
                        ...
                        echo "</a>";
                        if ($this->isLoggedIn) {
                            echo "<form method='POST' action='' class='relative' id='like-form-{$row['post_id']}'>";
                            echo "<input type='hidden' name='post_id' value='" . $row['post_id'] . "'>";
                            echo "<button type='submit' name='like' class='like-btn absolute bottom-10 right-10 bg-none border-none cursor-pointer'>";
                            echo "<svg xmlns='http://www.w3.org/2000/svg' fill='" . ($isLiked ? 'red' : 'none') . "' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' class='w-8 h-8'>";
                            echo "<path stroke-linecap='round' stroke-linejoin='round' d='M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z' />";
                            echo "</svg>";
                            echo "</button>";
                            echo "</form>";
                        }
