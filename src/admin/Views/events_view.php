<?php
include __DIR__ . '/../logic/events_logic.php';
...
<div class="events-list mt-6 mx-auto w-4/6 space-y-6">
    <?php if ($events->num_rows > 0): ?>
        <?php while ($event = $events->fetch_assoc()): ?>
            <div class="event bg-white shadow-lg rounded-lg p-6 relative">
                <h3 class="text-xl font-semibold text-gray-700"><?= htmlspecialchars($event['event_name']); ?></h3>
                <p class="text-gray-600 mt-2"><strong>Date:</strong> <?= htmlspecialchars($event['event_date']); ?></p>
                <p class="text-gray-600"><strong>Location:</strong> <?= htmlspecialchars($event['location']); ?></p>
                <p class="text-gray-600 mt-2"><strong>Description:</strong> <?= nl2br(htmlspecialchars($event['event_description'])); ?></p>
                <div class="actions absolute bottom-4 right-4 flex space-x-2">
                    <!-- Przycisk edycji -->
                    <button class="edit-btn bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition duration-200" 
                            onclick="openEditPopup(<?= $event['event_id']; ?>)">Edit</button>

                    <!-- Formularz usunięcia -->
                    <form method="POST" action="../logic/events_logic.php">
